...
Wdi <- WDI(country = Countries,
           indicator = c('NY.GDP.PCAP.PP.KD', 'NY.GDP.PCAP.KD.ZG', 
                         'BN.CAB.XOKA.GD.ZS', 'BM.GSR.GNFS.CD', 
                         'BX.GSR.GNFS.CD', 'FI.RES.TOTL.DT.ZS'),
           start = 1970, end = 2012)
names(Wdi) <- c('iso2c', 'country', 'year', 'GDPperCapita', 'GDPChange', 
                'CurrentAccount', 'Imports', 'Exports', 'Reserves')
...
# CurrentAccount 1
Wdi <- slide(Wdi, Var = 'CurrentAccount', GroupVar = 'country', 
             NewVar = 'CurrentAccountLag1')
# CurrentAccount 2
Wdi$CurrentAccountMinus <- Wdi$CurrentAccount - Wdi$CurrentAccountLag1 
# ChangeTermsTrade
Wdi$Terms <- Wdi$Exports/Wdi$Imports
Wdi <- PercChange(Wdi, Var = 'Terms', GroupVar = 'country', 
                  NewVar = 'TermsChange', type = 'proportion')

WdiSlim <- Wdi[, c('iso2c', 'year', 'GDPperCapita', 'Income33', 'Growth33', 
                   'CurrentAccountLag1', 'CurrentAccountMinus',
...
write.dta(Comb, file = 'data/KeeferExtended.dta')

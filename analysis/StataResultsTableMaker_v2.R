...
CleanUp <- data.frame(
        ...
        to = c('(Beta) constant', 'Pr(y=0) constant', 
               ...
               "Instability_AVG_LAG3", "R2", "No. of Clusters", '', 'No. of Countries')
    )

#### Final clean up for the linear models ####
outputA <- FindReplace(outputA, Var = 'var', replaceData = CleanUp, exact = F)
outputA <- outputA[1:11, ]
names(outputA) <- c('', 'Keefer', 'HK', 'LV-Keefer', 'LV pre-2001', 'LV Full')

# Save as .tex table
print(xtable(outputA, dcolumn = TRUE, booktabs = TRUE), size = 'scriptsize',
